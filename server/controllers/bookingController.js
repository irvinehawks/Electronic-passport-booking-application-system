...
const getAllBookings = async (req, res) => {
  ...
  // NO AWAIT
  let result = Bookings.find(queryObject);
  ...
  const totalJobs = await Booking.countDocuments(queryObject); // instead of using the length as length indicates 10 items here and not all jobs
...
const updateBooking = async (req, res) => {
  ...
  const booking = await Booking.findOne({ _id: jobId });
  ...
  const updatedJob = await Booking.findOneAndUpdate(
    { _id: jobId },
    req.body,
    {
      new: true,
  ...
  res.status(StatusCodes.OK).json({ updatedBooking }); // for postman
};
...
const deleteBooking = async (req, res) => {
  ...
  res.status(StatusCodes.OK).json({ msg: "success! booking Removed" }); // for postman
};
